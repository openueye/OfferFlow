...
export default function AuthLayout({ children }) {
  ...
  useEffect(() => {
    try {
      ...
      // The splash remains visible when session storage is unavailable.
    }
  }, [])
  ...
  const handleEnter = useCallback(() => {
    try {
      ...
      // Entering the app should still work when session storage is unavailable.
    }
    setSplashDone(true)
  }, [])
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden bg-offer-dark light-ambient-container">
      ...
      <div
        className="pointer-events-none fixed -top-[280px] -left-[160px] w-[700px] h-[700px] opacity-80 dark:opacity-100"
        style={{ background: 'radial-gradient(circle, rgba(126,87,194,0.12) 0%, transparent 65%)' }}
      />
      {/* Ambient glow — bottom-right cyan */}
      <div
        className="pointer-events-none fixed -bottom-[240px] -right-[200px] w-[600px] h-[600px] opacity-60 dark:opacity-100"
        style={{ background: 'radial-gradient(circle, rgba(34,200,230,0.08) 0%, transparent 65%)' }}
      />
      {/* Subtle center ambient */}
      <div
        className="pointer-events-none fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] opacity-30 dark:opacity-40"
