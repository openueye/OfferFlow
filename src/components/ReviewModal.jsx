...
export default function ReviewModal({ open, review, onClose }) {
  const { jobs, addReview, updateReview, addToast } = useApp()
  ...
  const addAttachment = async () => {
    ...
    for (const file of selectedFiles) {
      ...
      console.log('[attachment before save]', {
        reviewId: rid,
        attachmentId: attId,
        name: file.name,
        size: file.size,
        ...
        >
          ...
          >
            <div className="bg-white/90 backdrop-blur-xl dark:bg-transparent dark:backdrop-filter-none rounded-[22px] w-full max-w-full min-w-0 flex flex-col flex-1 min-h-0">
              ...
              {/* Body */}
              <div
                className="flex-1 overflow-y-auto p-5 pt-6 pb-7 space-y-6"
              ...
              >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <Field label="面试轮次">
                        ...
                        >
                          {ROUNDS.map((o) => (
                            <option key={o} className="bg-gray-950">
                              {o}
                            </option>
                          ))}
                        </select>
                ...
                <Section
                  ...
                  action={
                    <button
                      onClick={addQuestion}
                      className="text-xs text-offer-accent hover:text-offer-primary transition-colors flex items-center gap-1"
                    >
                      <svg
                ...
                >
                  <div className="space-y-3">
                    {questions.length === 0 && (
                      <p className="text-sm text-offer-muted/50 text-center py-3">
                        暂无面试问题，点击"添加问题"开始记录
                      </p>
                    )}
                    {questions.map((q, idx) => (
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <Field label="回答满意度">
                              <div className="flex items-center gap-1 h-9">
                                <StarRating
                                  value={q.satisfaction}
                                  onChange={(v) => updateQuestion(q.id, 'satisfaction', v)}
                                />
                                <span className="text-xs text-offer-muted ml-1">
                                  {q.satisfaction}/5
                                </span>
                              </div>
                            </Field>
                            <Field label="涉及能力点">
                              <input
                                value={q.skills}
                                onChange={(e) => updateQuestion(q.id, 'skills', e.target.value)}
                                placeholder="如：Java, 分布式"
                                className="min-h-[40px] w-full rounded-xl border border-white/10 bg-white/[0.03] px-4 py-2.5 text-sm text-white placeholder:text-gray-500 outline-none transition-all duration-200 focus:border-purple-400/70 focus:ring-2 focus:ring-purple-500/20"
                              />
                            </Field>
                          </div>
                          <Field label="更好的回答">
                            <textarea
                              value={q.betterAnswer}
                              onChange={(e) => updateQuestion(q.id, 'betterAnswer', e.target.value)}
                              rows={2}
                              placeholder="事后思考，怎样回答会更好..."
                              className="min-h-[40px] w-full rounded-xl border border-white/10 bg-white/[0.03] px-4 py-2.5 text-sm text-white placeholder:text-gray-500 outline-none transition-all duration-200 focus:border-purple-400/70 focus:ring-2 focus:ring-purple-500/20 resize-none"
                            />
                          </Field>
                        </div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {TAG_OPTIONS.map((tag) => (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${tags.includes(tag) ? 'bg-offer-primary text-white border-offer-primary' : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-100'}`}
                      >
                        {tag}
                      </button>
                    ))}
                  ...
                  <div className="space-y-2">
                    ...
                    {improvements.map((imp) => (
                      <div key={imp.id} className="flex items-center gap-2">
                        <input
                          value={imp.action}
                          onChange={(e) => updateImprovement(imp.id, e.target.value)}
                          placeholder="具体的改进行动..."
                    ...
                    <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 dark:bg-white/[0.02] dark:border-white/[0.06]">
                      ...
                      {selectedFiles.length === 0 ? (
                        <div>
                          ...
                          <p className="text-[10px] text-offer-muted mt-1.5">
                        ...
                        <div className="space-y-2">
                          <div className="space-y-1 max-h-32 overflow-y-auto">
                            {selectedFiles.map((f, i) => (
                              <div key={i} className="flex items-center gap-2 text-sm">
                                ...
                                <span className="text-offer-muted text-xs shrink-0">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            ...
                            >
                              {FILE_CATEGORIES.map((c) => (
                                <option key={c} className="bg-gray-950">
                                  {c}
                                </option>
                              ))}
                            </select>
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="px-3 py-1.5 rounded-lg text-xs font-medium bg-offer-primary text-white hover:bg-offer-accent transition-colors"
                            >
                              添加到复盘
                            </button>
                            ...
                            <button
                              ...
                              className="btn-secondary px-3 py-1.5 rounded-lg text-xs font-medium text-offer-muted border border-white/10 hover:text-white transition-colors"
                            >
                              取消
                            </button>
                          </div>
                    ...
                    {attachments.length > 0 && (
                      <div className="space-y-2">
                        {attachments.map((att) => (
                          <div
                            key={att.id}
                            className="bg-slate-50 rounded-xl px-4 py-3 border border-slate-200 dark:bg-white/[0.02] dark:border-white/[0.06]"
                          >
                            <div className="flex items-start justify-between">
                              ...
                              <button
                                onClick={() => removeAttachmentFromList(att.id)}
...
function Section({ title, action, children }) {
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-xs font-semibold text-slate-500 dark:text-white/45 uppercase tracking-wider">
          {title}
        </h3>
        {action}
      </div>
  ...
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((star) => (
        ...
        >
          <svg
            className={`w-4 h-4 ${star <= value ? 'text-amber-400' : 'text-offer-muted/30'}`}
            fill="currentColor"
            viewBox="0 0 20 20"
          >
