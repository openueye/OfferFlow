...
export default function TrendReportModal({ open, onClose }) {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        try {
          ...
          // Ignore invalid local preferences and use the server-side configuration.
        }
      }
  ...
  return (
    ...
    >
      <div
        className="modal-panel border w-full max-w-lg mx-4 max-h-[80vh] min-h-0 flex flex-col shadow-2xl shadow-black/40"
            ...
            <div className="flex-1 overflow-y-auto p-5">
              {loading && (
                <div className="flex flex-col items-center justify-center py-12 gap-3 ai-pulse">
                  <svg
                    className="animate-spin h-8 w-8 text-offer-accent"
                    viewBox="0 0 24 24"
                    fill="none"
                  >
                    <circle
                  ...
                  <p className="text-sm text-offer-muted ai-dots">
                    AI 正在分析面试趋势<span>.</span>
                    <span>.</span>
                    <span>.</span>
                  </p>
                </div>
              ...
              {error && (
                <div className="py-12 text-center">
                  ...
                  <button
                    onClick={fetchTrends}
                    className="mt-3 px-4 py-2 rounded-xl text-sm font-medium text-offer-accent hover:text-white transition-colors"
                  >
                    重试
                  </button>
                </div>
              ...
              {report && !loading && !error && (
                <div className="space-y-5">
                  ...
                  {report.高频薄弱点?.length > 0 && (
                    <div>
                      <h3 className="text-xs font-semibold text-offer-muted uppercase tracking-wider mb-2">
                        高频薄弱点
                      </h3>
                      <div className="flex flex-wrap gap-2">
                        {report.高频薄弱点.map((item, i) => (
                          <span
                            key={i}
                            className="px-3 py-1.5 rounded-lg text-xs font-medium bg-red-500/10 text-red-400 border border-red-500/20"
                          >
                            {item}
                          </span>
                        ))}
                        ...
                        {Object.entries(report.questionDistribution).map(([type, count]) => (
                          <div key={type} className="flex items-center gap-2 text-sm">
                            ...
                            <div className="flex-1 h-4 rounded-full bg-white/5 overflow-hidden">
                              <div
                                className="h-full rounded-full bg-offer-primary/60"
                                style={{
                                  width: `${Math.min((count / Math.max(...Object.values(report.questionDistribution))) * 100, 100)}%`,
                                }}
                              />
                            </div>
