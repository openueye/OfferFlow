...
function resultStyle(result) {
  switch (result) {
    case '通过':
      return 'text-emerald-700 dark:text-emerald-400 bg-emerald-50 border-emerald-200'
    case '待定':
      return 'text-amber-700 dark:text-amber-400 bg-amber-50 border-amber-200'
    default:
      return 'text-red-700 dark:text-red-400 bg-red-50 border-red-200'
  }
}
...
export default function ReviewDetailModal({ open, review, onClose, onEdit, onDelete }) {
  ...
  const handleFocusIn = useCallback((e) => {
    ...
    card.style.setProperty(
      '--mouse-x',
      `${targetRect.left + targetRect.width / 2 - cardRect.left}px`
    )
    card.style.setProperty(
      '--mouse-y',
      `${targetRect.top + targetRect.height / 2 - cardRect.top}px`
    )
  }, [])
  ...
  const handleDeleteAttachment = async (attId) => {
    ...
    const updatedAttachments = normalizeAttachments(liveReview.attachments).filter(
      (a) => a.id !== attId
    )
  ...
  return (
    ...
    >
      <div
        className="modal-panel border w-full max-w-2xl mx-4 max-h-[90vh] min-h-0 flex flex-col shadow-2xl shadow-black/40"
          ...
          <div className="bg-white/90 backdrop-blur-xl dark:bg-transparent dark:backdrop-filter-none rounded-[22px] w-full max-w-full min-w-0 flex flex-col flex-1 min-h-0">
            ...
            <ModalHeader onClose={onClose}>
              <div className="flex flex-col items-center min-w-0">
                <div className="flex items-center gap-2 min-w-0">
                  <h2 className="truncate text-base font-semibold leading-normal text-slate-950 dark:text-white">
                    {liveReview.companyName}
                  </h2>
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-medium border shrink-0 ${resultStyle(liveReview.result)}`}
                  >
                    {liveReview.result}
                  </span>
                </div>
                <p className="truncate text-sm font-medium leading-normal text-slate-600 dark:text-slate-300">
                  {liveReview.jobTitle} · {liveReview.round} · {liveReview.interviewType}
                </p>
              </div>
            </ModalHeader>
            ...
            <div className="flex-1 overflow-y-auto p-5 pt-6 pb-7 space-y-6" onFocus={handleFocusIn}>
                  ...
                  <InfoRow
                    ...
                    value={
                      <span className="flex items-center gap-1">
                        {renderStars(liveReview.rating)}{' '}
                        <span className="text-offer-muted text-xs">({liveReview.rating}/5)</span>
                      </span>
                    }
                  />
                  {liveReview.interviewerInfo && (
                    <InfoRow label="面试官" value={liveReview.interviewerInfo} />
                  )}
                  {liveReview.strengths && (
                    <InfoRow
                      label="优势"
                      value={liveReview.strengths}
                      className="text-emerald-400"
                    />
                  )}
                  {liveReview.weaknesses && (
                    <InfoRow label="不足" value={liveReview.weaknesses} className="text-red-400" />
                  )}
                </div>
                {liveReview.note && (
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {questions.map((q, idx) => (
                      <div
                        key={q.id}
                        className="card-glow bg-slate-50 rounded-xl p-4 border border-slate-200 dark:bg-white/[0.02] dark:border-white/[0.06]"
                      >
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-xs text-offer-muted font-medium">
                            问题 {idx + 1}
                          </span>
                          {q.skills && (
                            <span className="text-[10px] text-offer-accent/70 bg-offer-primary/10 px-2 py-0.5 rounded-full">
                              {q.skills}
                            </span>
                          )}
                        </div>
                          ...
                          {q.satisfaction && (
                            <div className="flex items-center gap-2 text-xs">
                              ...
                              <span className="flex items-center gap-0.5">
                                {renderStars(q.satisfaction)}
                              </span>
                              <span className="text-offer-muted">({q.satisfaction}/5)</span>
                            </div>
                          )}
                          {q.betterAnswer && (
                            <DetailBlock label="更好的回答" value={q.betterAnswer} />
                          )}
                        </div>
                      </div>
                    ))}
                ...
                <div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {positiveTags.map((tag) => (
                      <span
                        key={tag}
                        className="px-2.5 py-1 rounded-lg text-xs font-medium bg-emerald-50 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                <div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {(negativeTags.length > 0 ? negativeTags : tags).map((tag) => (
                      <span
                        key={tag}
                        className="px-2.5 py-1 rounded-lg text-xs font-medium bg-red-50 text-red-700 dark:text-red-300 border border-red-200 dark:border-red-800"
                      >
                        {tag}
                      </span>
                    ))}
              ...
              {improvements.length > 0 && (
                <div>
                  <SectionTitle>
                    改进动作 ({improvements.filter((i) => i.action).length})
                  </SectionTitle>
                    ...
                    {improvements
                      ...
                      .map((imp) => (
                        <div key={imp.id} className="flex items-center gap-2 text-sm">
                          <div
                            className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 ${imp.done ? 'border-offer-primary bg-offer-primary' : 'border-offer-muted'}`}
                          >
                            {imp.done && (
                              <svg
                          ...
                          <span
                            className={imp.done ? 'text-offer-muted line-through' : 'text-white'}
                          >
                            {imp.action}
                          </span>
                        </div>
                      ...
                      >
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs text-offer-muted shrink-0">
                                {att.fileSize}
                              </span>
                            </div>
                            <div className="flex items-center gap-2 mt-1 text-xs text-offer-muted">
                              <span className="bg-offer-primary/10 text-offer-accent px-1.5 py-0.5 rounded">
                                {att.fileCategory}
                              </span>
                            ...
                            {editingDescId === att.id ? (
                              <div className="flex items-center gap-2 mt-1">
                                ...
                                />
                                <button
                                  onClick={() => saveEditDesc(att.id)}
                                  className="text-xs text-offer-accent hover:text-offer-primary"
                                >
                                  保存
                                </button>
                                <button
                                  onClick={cancelEditDesc}
                                  className="text-xs text-offer-muted hover:text-white"
                                >
                                  取消
                                </button>
                              </div>
                              ...
                              att.description && (
                                <p className="text-xs text-offer-muted/70 mt-1">
                                  {att.description}
                                </p>
                              )
                            )}
                        ...
                        <div className="flex items-center gap-2 mt-2 pt-2 border-t border-white/5">
                          <button
                            ...
                            className="text-xs text-offer-accent hover:text-offer-primary transition-colors"
                          >
                            预览
                          </button>
                          <button
                            onClick={() => startEditDesc(att)}
                            className="text-xs text-offer-muted hover:text-white transition-colors"
                          >
                            编辑说明
                          </button>
                          <button
                            onClick={() => setConfirmDeleteAttId(att.id)}
                            className="text-xs text-offer-muted hover:text-red-400 transition-colors"
                          >
                            删除
                          </button>
                        </div>
...
function renderStars(count) {
  return Array.from({ length: 5 }, (_, i) => (
    <svg
      key={i}
      className={`w-3 h-3 ${i < count ? 'text-amber-400' : 'text-offer-muted/20'}`}
      fill="currentColor"
      viewBox="0 0 20 20"
    >
...
function FileTypeBadge({ type }) {
  ...
  return (
    <span
      className={`text-[10px] font-bold px-1.5 py-0.5 rounded shrink-0 ${colorMap[type] || 'text-offer-muted bg-white/80'}`}
    >
      {type}
    </span>
  )
