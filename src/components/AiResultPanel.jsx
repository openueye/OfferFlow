    ...
    <GlowCard className="rounded-[22px] w-full max-w-full">
      <div className="bg-white/90 backdrop-blur-xl dark:bg-[rgba(20,20,25,0.85)] rounded-[22px] w-full max-w-full min-w-0">
        ...
        <div className="max-h-[60vh] overflow-y-auto p-5 space-y-5">
          ...
          <div>
            <h3 className="text-xs font-semibold text-offer-muted uppercase tracking-wider mb-3">
              多维评分
            </h3>
                ...
                <div key={key} className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      ...
                      >
                        <svg
                          className={`w-4 h-4 ${star <= (scores[key] || 0) ? 'text-amber-400' : 'text-offer-muted/30'}`}
                          fill="currentColor"
                          viewBox="0 0 20 20"
                        >
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                  <svg
                    className={`w-5 h-5 ${star <= rating ? 'text-amber-400' : 'text-offer-muted/30'}`}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
            ...
            {questions.length > 0 ? (
              <div className="space-y-2">
                {questions.map((q, idx) => (
                  <div
                    key={q.id || idx}
                    className="card-glow rounded-xl p-3 border border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-white/[0.02]"
                  >
                    <p className="text-xs text-offer-muted mb-1">
                      Q{idx + 1}: {q.question}
                    </p>
                    {q.betterAnswer && (
