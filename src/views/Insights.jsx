    ...
    >
      ...
      {payload.map((item, index) => (
        ...
        >
          ...
          <span style={{ color: 'var(--c-text)' }}>：{item.value}</span>
        </div>
      ))}
    </div>
  )
...
export default function Insights() {
  ...
  const data = useMemo(() => {
    ...
    const active = filteredJobs.filter((j) =>
      ['已投递', 'OA / 笔试', '一面中', '二面中', '三面中', '终面中'].includes(j.status)
    )
    ...
    const pendingReviewCount = filteredJobs.filter(
      (j) => hasInterviewExperience(j) && !reviews.some((r) => r.jobId === j.id)
    ).length

    // Conversion rates
    const replyRate = appliedCount > 0 ? Math.round((replied.length / appliedCount) * 100) : 0
    const interviewRate =
      appliedCount > 0 ? Math.round((interviewed.length / appliedCount) * 100) : 0
    ...
    const funnel = [
      ...
      { name: '收到回复', value: replied.length, fill: '#9575DE' },
      {
        ...
        value: filteredJobs.filter((j) =>
          ['OA / 笔试', '一面中', '二面中', '三面中', '终面中', 'Offer'].includes(j.status)
        ).length,
        fill: '#4FC3F7',
      },
    ...
    filteredJobs.forEach((j) => {
      ...
      if (Array.isArray(rounds) && rounds.length > 0) {
        rounds.forEach((r) => {
          if (r.round in roundCounts) roundCounts[r.round]++
        })
      } else {
        const s = j.status
    ...
    const roundPassRates = [
      {
        round: '一面',
        count: roundCounts['一面'],
        nextCount: roundCounts['二面'],
        passRate: roundCounts['一面'] > 0 ? roundCounts['二面'] / roundCounts['一面'] : 0,
      },
      {
        round: '二面',
        count: roundCounts['二面'],
        nextCount: roundCounts['三面'],
        passRate: roundCounts['二面'] > 0 ? roundCounts['三面'] / roundCounts['二面'] : 0,
      },
      {
        round: '三面',
        count: roundCounts['三面'],
        nextCount: roundCounts['终面'],
        passRate: roundCounts['三面'] > 0 ? roundCounts['终面'] / roundCounts['三面'] : 0,
      },
      {
        round: '终面',
        count: roundCounts['终面'],
        nextCount: offers.length,
    ...
    return {
      ...
      interviewedCount: totalRoundCount,
      interviewedPeopleCount: interviewed.length,
    ...
    <div className="px-6 py-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-5 mb-8">
        ...
        <MetricCard
          ...
          sub={`${data.interviewedPeopleCount} 岗位参与 · ${data.interviewRate}% 面试率`}
          onClick={() => setDetailOpen(true)}
        />
        <MetricCard
          label="总 Offer"
          value={data.offerCount}
          sub={`${data.offerRate}% Offer 率`}
          accent
        />
      ...
      <div className="card-modern p-6 mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
          ...
          <RateBar
            ...
            color="from-offer-primary to-offer-accent"
          />
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="text-offer-muted border-b border-white/[0.06]">
                  <th className="text-left py-3 pr-2 text-xs font-semibold uppercase tracking-wider text-offer-muted/80">
                    渠道
                  </th>
                  <th className="text-right px-2 py-3 text-xs font-semibold uppercase tracking-wider text-offer-muted/80">
                    投递
                  </th>
                  <th className="text-right px-2 py-3 text-xs font-semibold uppercase tracking-wider text-offer-muted/80">
                    回复
                  ...
                  <th className="text-right pl-2 py-3 text-xs font-semibold uppercase tracking-wider text-offer-muted/80">
                    回复率
                  </th>
                </tr>
              </thead>
              <tbody>
                {data.channels.map((ch) => (
                  ...
                  >
                    ...
                    <td className="text-right px-2 py-3 text-offer-muted">
                      {ch.interviewedPeople}
                    </td>
                    <td className="text-right px-2 py-3 text-offer-muted">{ch.interviewRounds}</td>
                    <td className="text-right px-2 py-3 text-offer-accent font-medium">
                      {ch.offered}
                    </td>
                ...
                {data.channels.length === 0 && (
                  <tr>
                    <td colSpan={7} className="text-center py-6 text-offer-muted">
                      暂无数据
                    </td>
                  </tr>
                )}
              ...
              <tbody>
                {data.resumeStats.map((rs) => (
                  ...
                  >
                    ...
                    <td className="text-right px-2 py-2.5 text-offer-muted">
                      {rs.interviewPeopleCount}
                    </td>
                    <td className="text-right px-2 py-2.5 text-offer-muted">
                      {rs.interviewRoundCount}
                    </td>
                    <td className="text-right px-2 py-2.5 text-offer-accent font-medium">
                      {rs.offerCount}
                    </td>
      ...
      <div className="card-modern p-6 mb-8 min-h-[380px]">
        ...
        {data.cityDistribution.length > 0 ? (
          <ResponsiveContainer
            width="100%"
            height={Math.max(200, data.cityDistribution.length * 44 + 20)}
          >
            <BarChart
          ...
          <div className="py-12 text-center text-offer-muted text-sm">暂无投递城市数据</div>
        )}
      </div>
...
function MetricCard({ label, value, sub, accent, onClick }) {
  return (
    <div
      onClick={onClick}
      className={`card-modern p-6 relative overflow-visible min-h-[148px] flex flex-col justify-between ${onClick ? 'cursor-pointer card-hover' : ''}`}
    >
      ...
      <div className="space-y-2">
        ...
        <p
          className={`text-3xl font-semibold leading-tight tracking-tight ${accent ? 'text-emerald-400' : 'text-white'}`}
        >
          {value}
        </p>
      </div>
...
function InterviewDetailModal({ open, onClose, stats, jobs, offerCount }) {
  ...
  const roundInfo = [
    {
      round: '一面',
      count: stats.roundCounts['一面'],
      nextCount: stats.roundCounts['二面'],
      passRate: stats.roundPassRates[0].passRate,
    },
    {
      round: '二面',
      count: stats.roundCounts['二面'],
      nextCount: stats.roundCounts['三面'],
      passRate: stats.roundPassRates[1].passRate,
    },
    {
      round: '三面',
      count: stats.roundCounts['三面'],
      nextCount: stats.roundCounts['终面'],
      passRate: stats.roundPassRates[2].passRate,
    },
    {
      round: '终面',
      count: stats.roundCounts['终面'],
      nextCount: offerCount,
      passRate: stats.roundPassRates[3].passRate,
    },
  ]
  ...
  return (
    ...
    >
      <div
        className="modal-panel border w-full max-w-3xl mx-4 max-h-[90vh] flex flex-col shadow-2xl shadow-black/40"
      ...
      >
        <GlowCard
          ...
          className="rounded-[22px] w-full max-w-full min-w-0 flex flex-col flex-1"
        >
          <div className="bg-white/90 backdrop-blur-xl dark:bg-transparent dark:backdrop-filter-none rounded-[22px] w-full max-w-full min-w-0 flex flex-col flex-1 min-h-0">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-6">
              ...
              {/* Per-round cards */}
              <div>
                <h3 className="text-xs font-semibold text-slate-500 dark:text-white/45 uppercase tracking-wider mb-3">
                  各轮次统计
                </h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {roundInfo.map((r) => (
                    <div
                      key={r.round}
                      className="card-glow bg-slate-50 border border-slate-200 rounded-xl p-4 dark:bg-white/[0.02] dark:border-white/[0.06]"
                    >
                      ...
                      <p className="text-lg font-bold text-slate-900 dark:text-white">
                        {r.count}{' '}
                        <span className="text-xs text-slate-500 dark:text-white/45 font-normal">
                          次
                        </span>
                      </p>
                      ...
                      <div className="mt-2 h-1.5 bg-white/[0.04] rounded-full overflow-hidden">
                        <div
                          className="h-full rounded-full bg-gradient-to-r from-offer-primary to-offer-accent transition-all"
                          style={{ width: `${Math.min(r.passRate * 100, 100)}%` }}
                        />
                      </div>
                      <p className="text-xs text-offer-accent mt-1">
                        通过率 {formatPct(r.passRate)}
                      </p>
                    </div>
                  ))}
                </div>
              ...
              <div className="card-glow bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 dark:bg-white/[0.02] dark:border-white/[0.06]">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-lg font-bold text-emerald-600 dark:text-emerald-400">
                    {formatPct(stats.offerConversionRate)}
                  </span>
                </div>
              </div>
                  ...
                  <table className="w-full text-xs">
                    <thead className="sticky top-0 bg-slate-100 dark:bg-gray-950">
                      <tr className="text-slate-500 dark:text-white/45 border-b border-slate-200 dark:border-white/[0.06]">
                        <th className="text-left py-3 pr-2 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-white/45">
                          公司
                        </th>
                        <th className="text-left px-2 py-3 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-white/45">
                          岗位
                        </th>
                        <th className="text-left px-2 py-3 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-white/45">
                          当前状态
                        </th>
                        <th className="text-right px-2 py-3 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-white/45">
                          已参与轮次
                        </th>
                        <th className="text-right px-2 py-3 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-white/45">
                          最高轮次
                        ...
                        <th className="text-right pl-2 py-3 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-white/45">
                          结束原因
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {jobs.map((j) => (
                        <tr
                          key={j.id}
                          className="border-b border-slate-200 dark:border-white/[0.06] hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors"
                        >
                          <td className="px-4 py-3 text-slate-900 dark:text-white font-medium whitespace-nowrap">
                            {j.companyName}
                          </td>
                          <td className="px-4 py-3 text-slate-600 dark:text-white/65 whitespace-nowrap">
                            {j.jobTitle}
                          </td>
                          <td className="px-4 py-3">
                            <span className="inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-medium whitespace-nowrap bg-purple-50 text-purple-700 border-purple-200 dark:bg-offer-primary/[0.15] dark:text-offer-accent dark:border-offer-primary/30">
                              {j.status}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-slate-600 dark:text-white/65 text-right">
                            {getRoundList(j)}
                          </td>
                          <td className="px-4 py-3 text-slate-600 dark:text-white/65 text-right">
                            {getHighestRound(j)}
                          ...
                          <td
                            className="px-4 py-3 text-slate-500 dark:text-white/55 text-right max-w-[140px] truncate"
                            title={j.endReason || '-'}
                          >
                            {j.endReason || '-'}
                          </td>
