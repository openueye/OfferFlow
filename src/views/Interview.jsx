            ...
            {['全部', '通过', '待定', '未通过'].map((f) => (
              <button
                ...
                className={`inline-flex items-center justify-center whitespace-nowrap rounded-full border px-4 py-2 text-sm font-medium transition-all duration-200 ${
                  resultFilter === f
                    ? f === '通过'
                      ? 'border-emerald-400/60 bg-emerald-600/25 text-white font-semibold shadow-sm shadow-emerald-950/20'
                      : f === '待定'
                        ? 'border-amber-400/60 bg-amber-600/25 text-white font-semibold shadow-sm shadow-amber-950/20'
                        : f === '未通过'
                          ? 'border-red-400/60 bg-red-600/25 text-white font-semibold shadow-sm shadow-red-950/20'
            ...
            >
              ...
              <div className="flex-1 flex flex-col gap-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0 space-y-1">
                    <div className="flex items-center gap-2">
                      <h3 className="truncate text-lg font-semibold leading-7 text-white">
                        {r.companyName}
                      </h3>
                      <span
                        className={`shrink-0 text-[10px] px-2 py-0.5 rounded-full font-medium border ${RESULT_STYLE[r.result] || 'text-offer-muted bg-black/40 border-white/10'}`}
                      >
                        {r.result}
                      </span>
                    </div>
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-2 text-sm text-offer-muted">
                  ...
                  <span className="bg-theme-hover px-2 py-0.5 rounded">
                    {r.interviewType || '技术面'}
                  </span>
                ...
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <svg
                      key={s}
                      className={`w-3.5 h-3.5 ${s <= (r.rating || 0) ? 'text-amber-400' : 'text-offer-muted/20'}`}
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
                ...
                {tags.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {tags.slice(0, 4).map((t) => (
                      <span
                        key={t}
                        className="text-[10px] bg-red-500/10 text-red-400 border border-red-500/20 px-1.5 py-0.5 rounded-full"
                      >
                        {t}
                      </span>
                    ))}
                    {tags.length > 4 && (
                      <span className="text-[10px] text-offer-muted">+{tags.length - 4}</span>
                    )}
                  </div>
                )}
              ...
              <div className="mt-auto flex items-center justify-between border-t border-white/[0.06] pt-4">
                <div className="flex items-center gap-3 text-sm text-offer-muted/80">
                  ...
                  {attachCount > 0 && (
                    <>
                      <span className="w-px h-3.5 bg-white/10" />
                      <span>附件 {attachCount}</span>
                    </>
                  )}
                </div>
                <button
                  onClick={() => setDetailReview(r)}
                  className="text-sm text-offer-accent hover:text-white transition-colors font-medium"
                >
                  查看详情
                </button>
              </div>
  ...
  return (
    <div className="card-modern p-4 mb-5">
      <div className="flex items-center gap-2 mb-3">
        <h3 className="text-xs font-semibold text-offer-muted uppercase tracking-wider">
          弱项标签分析
        </h3>
        <span className="text-[10px] text-offer-muted/50 bg-white/[0.03] px-2 py-0.5 rounded-full">
          {tagFreq.length} 个弱项标签 ·{' '}
          {reviews.filter((r) => (r.negativeTags || r.tags || []).length > 0).length} 场面试
        </span>
      </div>
      <div className="flex flex-wrap gap-2">
        {tagFreq.map(([tag, count], idx) => {
          ...
          const size =
            count === maxFreq && maxFreq > 1 ? 'text-sm px-3.5 py-1.5' : 'text-xs px-2.5 py-1'
          return (
            <button
              key={tag}
            ...
            >
              {tag}
              <span className={`text-[10px] ${isActive ? 'text-purple-200' : 'opacity-60'}`}>
                {count}
              </span>
            </button>
          )
